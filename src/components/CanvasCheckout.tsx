...
const CanvasCheckout: React.FC<CanvasCheckoutProps> = ({ design, onBack, onComplete }) => {
    ...
    // Función para dibujar el canvas superior (diseño)
    const drawUpperCanvas = useCallback(() => {
        if (!upperCanvasRef.current || !design.selectedDesign) return;
        ...
        designImg.onload = () => {
            ...
            const size = Math.min(canvas.width, canvas.height) * 0.15 * (
                design.designSize === 'small' ? 0.6 :
                    design.designSize === 'medium' ? 0.8 : 1.0
            );
