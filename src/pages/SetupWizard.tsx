...
const SetupWizard: React.FC = () => {
  ...
  const handleSaveDtfCost = async () => {
    ...
    try {
      ...
      toast.success('Costo de DTF actualizado', 'El valor se guardó correctamente');
    } catch (error) {
      console.error('Error saving DTF cost:', error);
      toast.error('Error', 'No se pudo guardar el costo de DTF');
    } finally {
  ...
  const handleSaveShippingCost = async () => {
    ...
    try {
      await ConfigService.updateShippingCost(shippingCost);
      toast.success('Costo de envío actualizado', 'El valor se guardó correctamente');
    } catch (error) {
      console.error('Error saving shipping cost:', error);
      toast.error('Error', 'No se pudo guardar el costo de envío');
    } finally {
    ...
    <div className="space-y-6">
          ...
          <div className="flex-1">
              ...
              <div className="bg-gray-700 rounded-lg p-4">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button
                    ...
                    className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors disabled:bg-gray-600 disabled:cursor-not-allowed font-medium"
                  >
                    {savingShippingCost ? 'Guardando...' : 'Guardar'}
                  </button>
                </div>
                <p className="text-xs text-gray-400 mt-2">
                  Este costo se mostrará al cliente en el detalle del producto como costo de envío estimado. El costo real de envío se puede ajustar individualmente en cada pedido.
                </p>
              </div>
            </div>
            ...
            <div className="bg-yellow-900/30 border border-yellow-700 rounded-lg p-3 mb-3">
              <p className="text-xs text-yellow-300">
                <strong>💡 Tip:</strong> Los costos de Mano de Obra y Envío final se configuran individualmente para cada pedido desde la gestión de pedidos.
              </p>
            </div>
      ...
      <div className="bg-blue-900/30 border border-blue-700 rounded-lg p-4">
        ...
        <ul className="text-sm text-blue-300 space-y-1 list-disc list-inside">
          ...
          <li>El costo de envío estimado se muestra a los clientes en el detalle del producto</li>
          <li>Los costos de Mano de Obra y Envío final se ajustan por pedido desde la gestión de pedidos</li>
          <li>El costo de producción de cada producto se configura en el menú de Productos</li>
          <li>Puedes modificar estos costos en cualquier momento</li>
        </ul>
      </div>
    </div>
  );
